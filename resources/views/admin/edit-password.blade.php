...
@section('title') Dashboard Edit Password - Admin @endsection

@section('menu')
    <a class="nav-link dashboard aktif" href="{{ url('/admin/dashboard/teacher/list') }}">
...
@section('content')
    ...
    <a href="{{ route('teacher.details', $teacher->id) }}" class="btn btn-outline-info mt-4">
...
<div class="card mt-4">
    ...
    <div class="card-body">
        <form action="/admin/dashboard/teacher/{{ $teacher->user->id }}/change-password/update" method="POST">
            @csrf

            <div class="form-group col-md-6">
                <label for="inputPassword" class="col-form-label">Password Baru</label>
                <input name="password" type="text" class="form-control @error('password') is-invalid @enderror" id="inputPassword">
                @error('password')
